      ...
      <div className="flex px-4 space-x-3 sm:space-x-0 sm:px-3 sm:h-full sm:flex-col sm:flex-grow sm:justify-between sm:items-center sm:py-4">
        ...
        <div className="flex justify-around items-center w-1/5 sm:w-auto sm:block sm:space-y-2">
          <div className={`${showSettings ? 'flex' : 'hidden'} justify-around items-center absolute bottom-16 left-1/2 transform -translate-x-1/2 space-x-3 sm:space-x-0 sm:transform-none px-3 py-2 rounded-t-md sm:p-0 bg-gray-100 dark:bg-gray-800 sm:static sm:w-auto ${showSettings ? 'sm:block' : 'hidden'} sm:space-y-2`}>
            <div onClick={(e) => {e.target === e.currentTarget && setShowForm(false); submitInProgress && setSubmitInProgress(false); submitSuccess && setSubmitSuccess(false)}} className={`${showForm ? 'flex' : 'hidden'} justify-center items-center fixed z-20 inset-0 h-screen w-screen bg-gray-700 bg-opacity-70`}>
              <form onSubmit={handleSubmit} name="contact" method="post" data-netlify="true" data-netlify-honeypot="bot-field" className="w-full max-w-lg bg-white rounded px-10 py-8">
                ...
                <div className={`${submitSuccess ? 'hidden' : 'block'}`}>
                  <label>
                  ...
                  <button type="submit" className="flex justify-center items-center space-x-2 mt-4 w-full bg-gray-600 py-3 rounded font-bold text-white cursor-pointer hover:bg-gray-800 focus:outline-none" type="submit">
                    {!submitInProgress && <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="feather feather-send h-5 w-5"><line x1="22" y1="2" x2="11" y2="13"></line><polygon points="22 2 15 22 11 13 2 9 22 2"></polygon></svg>}
                    {submitInProgress && <svg class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                ...
                <div className="flex justify-center items-center space-x-1">
                  ...
                  <span className="text-green-600 text-3xl font-bold">Sucess</span>
                </div>}
              </form>
            </div>
